...
def build_pdf_report(title_text: str, brief_data: dict) -> "io.BytesIO":
    ...
    story.append(Paragraph(escape_xml_text(title_text), title_style))
    ...
    overview_data = [
        ...
        [Paragraph("Critical Critique:", meta_label_style), Paragraph(escape_xml_text(flaw), meta_val_style)],
        [Paragraph("Key Takeaways:", meta_label_style), Paragraph(escape_xml_text(brief_data.get("abstract_summary", "")), meta_val_style)]
    ]
    ...
    story.append(Paragraph(escape_xml_text(brief_data.get("methodology", "")), body_style))
    story.append(Spacer(1, 10))
    
    story.append(Paragraph("2. Experimental Results & Benchmarks", h1_style))
    story.append(Paragraph(escape_xml_text(brief_data.get("results", "")), body_style))
    story.append(Spacer(1, 10))
    
    story.append(Paragraph("3. Assumptions, Limitations & Future Scope", h1_style))
    story.append(Paragraph(escape_xml_text(brief_data.get("limitations", "")), body_style))
    ...
    bg_nodes = [escape_xml_text(n.get("label", "")) for n in nodes if n.get("group", "").lower() in ["background", "default"]]
    arch_nodes = [escape_xml_text(n.get("label", "")) for n in nodes if n.get("group", "").lower() in ["architecture", "model"]]
    method_nodes = [escape_xml_text(n.get("label", "")) for n in nodes if n.get("group", "").lower() == "methodology"]
    res_nodes = [escape_xml_text(n.get("label", "")) for n in nodes if n.get("group", "").lower() == "results"]
    ...
    for c_idx, card_item in enumerate(cards):
        q = f"Q{c_idx+1}: {escape_xml_text(card_item.get('question', ''))}"
        a = f"A: {escape_xml_text(card_item.get('answer', ''))} (Page {card_item.get('page_number', 'N/A')})"
